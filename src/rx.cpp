...
// Well-known hash key: packet dedup hashes must match between forwarder and aggregator hosts
static const uint8_t dedup_hash_key[crypto_shorthash_KEYBYTES] = { 'w', 'f', 'b', '-', 'n', 'g', ' ',
                                                                   'd', 'e', 'd', 'u', 'p', ' ',
                                                                   'k', 'e', 'y' };

static uint64_t packet_hash(const uint8_t *buf, size_t size)
{
    uint64_t pkt_hash;

    static_assert(sizeof(pkt_hash) == crypto_shorthash_BYTES, "Invalid packet hash size");
    crypto_shorthash((uint8_t*)&pkt_hash, buf, size, dedup_hash_key);
    return pkt_hash;
}
...
{
    if (dedup_batch_size > 1 && get_time_ms() >= dedup_batch_flush_ts)
    {
        flush_dedup_batch();
    }
    ...
    // Deduplicate data packets received by multiple adapters.
    // Session packets are small, rare and aggregator drops extra copies itself,
    // so forward them as is.
    if (size > 0 && buf[0] == WFB_PACKET_DATA)
    {
        uint64_t pkt_hash = packet_hash(buf, size);

        if (seen_packets.contains(pkt_hash))
        {
            // Enqueue stats record instead of forwarding duplicate packet
            wrxfwd_dedup_t rec;

            memcpy(rec.pkt_hash, &pkt_hash, sizeof(rec.pkt_hash));
            rec.rx_info = fwd_hdr;

            if (dedup_batch_size + sizeof(rec) > sizeof(dedup_batch))
            {
                flush_dedup_batch();
            }

            if (dedup_batch_size == 1)
            {
                dedup_batch_flush_ts = get_time_ms() + FWD_DEDUP_BATCH_MS;
            }

            memcpy(dedup_batch + dedup_batch_size, &rec, sizeof(rec));
            dedup_batch_size += sizeof(rec);
            return;
        }

        seen_packets.insert(pkt_hash);
    }
...
void Forwarder::flush_dedup_batch(void)
{
    if (dedup_batch_size <= 1) return;

    // wlan_idx = 0xff marks packet generated by forwarder itself,
    // relayed radio packets always have real wlan_idx
    wrxfwd_t fwd_hdr = { .wlan_idx = 0xff,
                         .freq = 0,
                         .mcs_index = 0,
                         .bandwidth = 0 };
    ...
    struct iovec iov[2] = {{ .iov_base = (void*)&fwd_hdr,
                             .iov_len = sizeof(fwd_hdr)},
                           { .iov_base = (void*)dedup_batch,
                             .iov_len = dedup_batch_size }};
...
void Forwarder::dump_stats(void)
{
    flush_dedup_batch();
}
                                ...
                                uint8_t bandwidth, sockaddr_in *sockaddr)
{
    ...
    case WFB_PACKET_SESSION:
        ...
        // Also session packet content doesn't change during session lifetime, so an attacker
        // can capture one and replay it many times, creating fake antenna statistics.
    ...
    case WFB_PACKET_DEDUP_STATS:
    {
        // Batch of stats records for packets deduplicated by forwarder.
        // wlan_idx = 0xff means that packet was generated by forwarder itself
        // and was not relayed from the air.
        if (wlan_idx != 0xff || size < 1 + sizeof(wrxfwd_dedup_t) || (size - 1) % sizeof(wrxfwd_dedup_t) != 0)
        {
            WFB_ERR("Invalid dedup stats packet\n");
            count_p_bad += 1;
            return;
        }

        // Batch itself is not a packet copy - count accepted records instead
        count_p_all -= 1;

        for(size_t offset = 1; offset < size; offset += sizeof(wrxfwd_dedup_t))
        {
            wrxfwd_dedup_t rec;
            uint64_t pkt_hash;

            memcpy(&rec, buf + offset, sizeof(rec));
            memcpy(&pkt_hash, rec.pkt_hash, sizeof(pkt_hash));

            // Count stats only for packets that were successfully decrypted locally
            if (!decrypted_packets.contains(pkt_hash)) continue;

            count_p_all += 1;
            count_p_uniq.insert(pkt_hash);
            log_rssi(sockaddr, rec.rx_info.wlan_idx, rec.rx_info.antenna, rec.rx_info.rssi, rec.rx_info.noise,
                     ntohs(rec.rx_info.freq), rec.rx_info.mcs_index, rec.rx_info.bandwidth);
        }
        return;
    }
    ...
    if (sockaddr != NULL)
    {
        // Remember packet hash to validate dedup stats records from forwarders
        decrypted_packets.insert(pkt_hash);
    }
